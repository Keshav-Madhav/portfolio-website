...
const copilotTool = {
  id: "verbaflo-copilot",
  title: "VerbaFlo Copilot — agentic talk-to-any-data-source system",
  content: `
Topics covered here: verbaflo copilot, verbaiq, agentic copilot, agentic
harness, self-directing brain, worker clones, delegate tool, talk to your
data, text to sql, t2s, mongodb postgres milvus, parallel agents, prompt
caching, the tool keshav is most proud of.

The **Copilot** (productized as **VerbaIQ**) is the VerbaFlo tool Keshav is
most proud of. It started as a multi-agent "talk-to-any-data-source" system
on a triage → specialists → analyst architecture, and he has since **rebuilt
it into a fully agentic harness** — a single self-directing "brain" modeled
on how Claude Code itself works.

**The current (agentic) architecture:**
- A single **brain** (default Claude Sonnet 4.6) plans the work, then spawns
  task-scoped **worker clones** in parallel via a \`delegate()\` tool — instead
  of a fixed roster of specialists.
- Each worker clone (default Claude Haiku 4.5) is a generic, source-
  parameterized agent configured at spawn time for \`mongo\`, \`sql\`, or \`faq\`.
- The brain also **researches** its own data paths (RAG over a reference
  corpus), does exact arithmetic via a \`compute\` tool, keeps a TodoWrite-style
  plan, persists scratch files to a **session workspace**, consults a file-
  level documentation **wiki**, and then **finalizes** a cited answer.
- Roughly **25 tools** across five families: Mongo query, SQL/Postgres query,
  Milvus FAQ search, RAG research, a file-level wiki, and a scratch workspace.

It runs **strictly read-only and tenant-isolated by construction**: Postgres
is DB-level read-only plus SQL validation; Mongo write stages ($out/$merge)
are rejected; per-client collection naming and non-bypassable tenant filters
are invariants the LLM cannot override. The brain can reach ANY of a client's
MongoDB collections or Postgres tables (with runtime list/schema discovery),
not a fixed allowlist — "full-freedom data exploration."

Up to **8 worker clones** run in parallel (semaphore-bounded), with per-worker
(45s), per-step (120s), and per-turn (240s) timeouts plus a per-collection
circuit breaker. Everything streams over SSE: the brain streams its reasoning
token-by-token and emits typed events (plan, thinking, agent_start/complete,
tool_call, delegate, research, compute, wiki, workspace, finalize) so a React
activity panel renders the brain "thinking" live.

**Cost engineering:** aggressive Anthropic prompt-cache placement cut measured
warm-turn cost ~**73% (≈$0.139 → ≈$0.037)** with **83–95% prompt-cache reads**,
with cache-aware per-turn token/cost accounting surfaced in the UI.

**Its own documentation/wiki:** the agent reads ~20,000 lines of reference docs
(MongoDB + PMS schemas, API, frontend) — one of which, a ~9,900-line PostgreSQL
schema reference, was itself generated by a 14-agent workflow and indexed into
441 RAG chunks. The wiki is reachable two ways: semantic/BM25 research, and
direct \`wiki_grep\`/\`wiki_read\` from disk.

How it was built (the story):
1. Started as a tiny side experiment — proving sub-second text-to-SQL was
   achievable with a single Postgres specialist.
2. Grew into a triage → parallel specialists → analyst pipeline (8 Mongo
   specialists + Milvus FAQ + Postgres), with SSE streaming, a semantic cache
   (~40% hit rate), schema pre-warming, and query expansion.
3. Then Keshav rewrote the rigid 13-agent state machine into the agentic brain
   above — additive and feature-flagged, with the legacy pipeline kept as a
   fallback and a separate session-memory namespace so the two never mix.

A sub-second text-to-SQL spin-off is also used directly INSIDE the primary
conversational bot for Postgres-backed queries.

Not a framework wrapper. Custom runtime — not LangGraph, not CrewAI. Models are
...
Keshav is the dominant author (~74% of the codebase by line count) and built
most of the production infrastructure around the replay core: a single-pool
Chrome architecture that cut 8 concurrent replays from ~7.9 GB to 2.5 GB of
RAM (killing a 32% bot-timeout rate), an S3 auto-update system with stable +
pre-release channels, Google-OAuth admin, an Elasticsearch tracing layer, PII
safety markers, and a built-in MCP "Investigate" server that exposes
transcripts + traces to Claude Code / Cursor for AI-assisted RCA. (The original
persona/replay core and the multi-modality SMS/WhatsApp/call engines were
authored by teammates; Keshav hardened the core and owns the surrounding infra.)

Tech stack: Electron (desktop shell), Puppeteer/CDP (widget automation),
MongoDB (transcript storage), Elasticsearch (trace enrichment), MCP SDK,
AWS S3 + Secrets Manager, custom LiteLLM wrapper (to hit any model), React (UI).
`,
};
...
const tracingTool = {
  id: "verbaflo-tracing",
  title: "VerbaFlo LLM Engineering Platform (verbaflo-playground)",
  content: `
Topics covered here: verbaflo playground, llm engineering platform, tracing
ui, llm playground, trace explorer, service replay, prompt evaluation, git
sync prompts, model comparison, observability, keshav built the playground.

The **LLM Engineering Platform** (repo: \`verbaflo-playground\`) is VerbaFlo's
own purpose-built suite for observing, debugging, replaying, evaluating, and
improving the prompts behind their production conversational agents — using
real production traffic as the test bed. Keshav is the **primary author (~85%
of commits)**; it started as a "benchmark prompt changes against production"
tool and grew into a full platform.

What it does:
- **Trace Explorer**: Elasticsearch-backed observability — filter by user,
  time, stage, latency, cost, error; waterfall + flow-graph views,
  conversations-by-lead, tool-calls view, span errors, statistics.
- **LLM Playground**: Monaco editors with live server-side Jinja2 rendering;
  **compare up to 4 models side-by-side** in one run, routing to **14
  providers** (OpenAI, Anthropic, Gemini, Groq, Cerebras, Azure, Mistral,
  Together, DeepSeek, xAI, Cohere, Fireworks, OpenRouter, Perplexity) via
  LiteLLM. "Open in Playground" prefills any captured production call for edits.
- **Bulk / Batch runs**: re-run a new prompt against 5–500 historical traces
  matched by \`prompt_key\`, with per-row diffs and LLM-judge similarity/
  correctness scoring.
- **Service Replay**: re-fire historical production traces against a live
  deployed service (DEV→PROD) — up to **20,000 traces in one sweep**, with
  \`runs/trace\` for hallucination/nondeterminism detection.
- **Tool-calling agent loop**: executes a model's tool calls against live
  \`vfai-bakery\` deploys, mirroring exactly what production does.
- **Git Sync**: edit a prompt in the browser and commit / branch / open a PR
  straight to the \`vf-prompts\` repo (validated Jinja + config, S3-audited).
- **Vault** (saved-trace snapshots with threaded comments), a tool-using **AI
  Assistant**, **Call Debugging** for voice (Vapi), and **VerbaSuper telemetry**
  dashboards.

Why in-house instead of Opik / LangSmith / Langfuse: off-the-shelf tools were
too slow for VerbaFlo's query patterns (hundreds of thousands of conversations
per week) and didn't model VerbaFlo's specific pipeline stages (Router, FAQ,
Text2SQL, PMS, bot).

Scale: a **236-endpoint FastAPI backend** sharing a pure-Python \`logic/\` core
with a **Next.js 16 / React 19** frontend (~190 React components) — roughly
**170K lines of code** in total. GitHub-OAuth auth with Mongo sliding sessions,
a Celery/Redis background-jobs subsystem, and AWS Secrets Manager.

Built on: Python / FastAPI (backend), Elasticsearch + MongoDB + Opik (data),
LiteLLM (14 providers), Next.js 16 / React 19 + Monaco + Recharts + Mermaid
(frontend). Shares trace data with the simulation MCP and the copilot's
kibana_trace.
`,
};
...
const projectVfCopilotDeepDive = {
  ...
  title: "VerbaFlo Copilot — Agentic Data-Exploration Harness (Deep Dive)",
  content: `
Topics covered here: vf-copilot, verbaiq, agentic copilot, agentic harness,
self-directing brain, worker clones, delegate tool, multi-agent, copilot,
internal tool, mongodb query, postgres query, milvus, faq search, analyst
agent, text to sql, t2s, natural language to database, prompt caching,
read-only tenant isolation, verbaflo internal.

**vf-copilot** (productized as **VerbaIQ**) is VerbaFlo's internal system for
answering natural-language questions over the company's CRM/PMS data. It began
as a triage → parallel specialists → analyst pipeline and Keshav **rebuilt it
into a fully agentic harness**: a single self-directing brain that plans,
delegates to parallel worker clones, and self-concludes.

**THE CUSTOM HARNESS (built from scratch):**
The thing Keshav is proudest of here is that the agent loop is *his own harness*,
not a framework. No LangGraph, no CrewAI, no AutoGen — he wrote the plan →
delegate → reflect → finalize loop himself, modeled on how Claude Code works,
so he controls every step, timeout, and guardrail. What the harness gives the
model:
- **A read-only environment (safety sandbox).** The agent has genuine freedom to
  roam the data, but the environment itself makes destructive action impossible:
  Postgres is opened DB-level read-only with SQL validation on top; Mongo write
  stages ($out/$merge) are rejected; tenant-isolation filters are injected and
  non-bypassable. The model literally cannot mutate or cross-tenant-leak data,
  so it can be given free rein without risk.
- **Thinking freedom.** Extended/streamed reasoning with a configurable thinking
  budget — the brain reasons before and between tool calls, and that reasoning
  streams to the UI so you can watch it think.
- **Self-managed task lists.** A TodoWrite-style \`update_plan\` tool lets the agent
  write, re-order, and check off its own plan mid-run, like a developer keeping a
  checklist — instead of following a fixed scripted flow.
- **Multiple spawns + sub-agents.** A \`delegate()\` tool spawns task-scoped
  sub-agents (worker clones) that run **in parallel** (semaphore-bounded at 8),
  each configured for a data source at spawn time. The brain fans work out,
  collects results, and reasons over them — a real orchestrator/sub-agent
  hierarchy it builds on the fly, not a hardcoded roster.
- **Its own scratch-workspace + wiki.** A session filesystem it can write and read
  intermediate results to, plus a file-level documentation wiki it can \`grep\`
  and read on demand to learn the schema before it queries.

**Agentic architecture (current):**
\`\`\`
User Question
     ↓
  Brain (Claude Sonnet 4.6) — plans, reasons, streams
     ├─ delegate() ──► DataWorker clones (Haiku 4.5), spawned in PARALLEL
     │                  • mongo  • sql  • faq   (source-parameterized)
     ├─ research() ──► RAG over ~20k lines of reference docs
     ├─ wiki_grep / wiki_read ──► file-level schema/API wiki
     ├─ workspace_* ──► session scratch filesystem
     ├─ compute() ──► exact arithmetic
     └─ finalize() ──► cited answer, streamed over SSE
\`\`\`
The legacy pipeline (Triage → 8 Mongo specialists + Milvus FAQ + Postgres →
Analyst, ~13 agents) is kept as a feature-flagged fallback with its own
session-memory namespace.

**Tool roster (~25 tools, five families):**
- Mongo: find_documents, aggregate_documents, get_distinct_values,
  get_document_by_id, list_collections.
- SQL/Postgres: execute_sql, get_table_schema, get_distinct_column_values,
  count_records, list_tables.
- FAQ/Milvus: search_faq, lookup_property_ids (+ hybrid/dense variants).
- RAG research: keyword_search (BM25), semantic_search (dense), read_chunk.
- Wiki + workspace: wiki_toc/grep/read, workspace_list/write/read/splice/delete.

**Guarantees (invariants the LLM cannot override):**
- **Read-only**: DB-level read-only Postgres + SQL validation; Mongo
  $out/$merge rejected.
- **Tenant isolation**: per-client collection naming + non-bypassable filters
  (with a deliberate cross-client mode for org-wide analysis).
- **Bounded parallelism**: up to 8 concurrent worker clones (semaphore), with
  per-worker (45s), per-step (120s), and per-turn (240s) timeouts and a
  per-collection circuit breaker.

**Technical Stack:**
- FastAPI (async), Pydantic v2, Jinja2 prompts.
- Motor/pymongo (3 separate Mongo connections), asyncpg (Postgres),
  pymilvus 2.6 (dense + BM25 sparse).
- LiteLLM + Instructor — multi-provider; default Claude Sonnet 4.6 (brain) +
  Haiku 4.5 (workers), swappable to GPT/Opus 4.8 via env.
- In-house \`kibana_trace\` observability (one Elasticsearch span doc per traced
  call), replacing the previous Opik tracer.
- Docker / Kubernetes (NGINX ingress tuned for long-lived SSE), AWS Secrets
  Manager.

**Engineering highlights:**
1. **Cost**: prompt-cache placement cut warm-turn cost ~73% (≈$0.139 →
   ≈$0.037) at 83–95% cache reads, with cache-aware per-turn accounting in UI.
2. **Its own wiki**: ~20k lines of reference docs (Mongo + PMS schemas, API,
   frontend); the ~9.9k-line PMS reference was generated by a 14-agent workflow
   and indexed into 441 RAG chunks.
3. **SSE streaming**: token-by-token reasoning with extended-thinking support,
   a live plan checklist, and mid-stream client-disconnect cancellation.
4. **Eval discipline**: a 121-item adversarial capability exam plus a
   side-by-side agentic-vs-legacy harness with an LLM judge.
...
**What Keshav Built:**
The entire agentic engine is his — the brain/runner/workers, all the tools
(delegate, research, compute, wiki, workspace), the kibana_trace package, the
SSE streaming + disconnect handling, the prompt-cache cost work, the capability
exam + eval harnesses, the mined reference wiki, and the React activity UI —
on top of the original text-to-SQL and pipeline work he also authored.

Note: the legacy "MCP" in copilot's older docs meant "Multi-agent Conversation
...
**vf-copilot (Agentic Data-Exploration System / VerbaIQ):**
- **Built from the ground up** (Dec 2025 initial commit)
- Designed the original multi-agent pipeline (Text-to-SQL, Milvus FAQ,
  streaming, React UI, memory + caching)
- **Rewrote it into a fully agentic harness**: a self-directing brain that
  spawns parallel worker clones via a delegate tool, with ~25 tools, a session
  workspace, and its own technical wiki — read-only and tenant-isolated
- Cut warm-turn LLM cost ~73% via prompt-cache placement (83–95% cache reads)

**vf-simulation (Replay Harness + MCP Investigator):**
- Hardened the replay core and built most of the surrounding infra (owns ~74%)
- **Single-pool Chrome architecture**: ~3× resource cut (7.9 GB → 2.5 GB)
- Built the **MCP "Investigate" server** for AI-assisted RCA from Claude/Cursor
- Google-OAuth admin, S3 dual-channel auto-update, ES tracing, PII safety markers

**verbaflo-playground (LLM Engineering Platform):**
- **Primary author (~85% of commits)** — grew it from an eval toolkit into a
  full platform (~170K LOC, 236 FastAPI endpoints + Next.js 16 / React 19)
- LLM Judge eval, Trace Explorer, 4-way model compare across 14 providers
- **Service Replay** (up to 20,000 production traces), tool-calling agent loop
- **Git-Sync prompts→PR**, the saved-trace Vault, and a tool-using AI assistant
...
**Pipeline (pre-rewrite) Architecture:**
\`\`\`
...
**MID-2026: THE AGENTIC REWRITE (the big one)**

Keshav tore out the hardcoded 13-agent state machine and rebuilt the Copilot
as a **fully agentic harness** modeled on how Claude Code works — a single
self-directing brain instead of a fixed router + specialists. Shipped as an
additive, feature-flagged engine (legacy pipeline kept as fallback, separate
session-memory namespace). The agentic rewrite landed as ~141 files changed /
~77K insertions (much of it generated reference docs).

What he built:
- **The brain + runner loop** (\`app/services/agentic/copilot_runner.py\`): plan →
  delegate (parallel) → reflect → finalize, emitting typed SSE events. Brain =
  Claude Sonnet 4.6, system prompt rebuilt each turn from a capability manifest.
- **Worker clones** (\`workers.py\`): one generic source-parameterized DataWorker
  (\`mongo\`|\`sql\`|\`faq\`, default Haiku 4.5) replacing the 8 hardcoded Mongo
  agents, plus a ResearchWorker. Spawned in parallel via asyncio + a semaphore
  (max 8), with per-worker/step/turn timeouts and a per-collection circuit
  breaker.
- **A new tool layer** (\`agentic_tools.py\`, \`wiki_tools.py\`, \`workspace_tools.py\`,
  \`reference_search_tools.py\`): \`delegate\`, \`research\`, \`update_plan\`, \`compute\`,
  \`finalize\`, \`ask_user\`, file-level wiki (\`wiki_toc/grep/read\`), and a session
  scratch-workspace (\`workspace_list/write/read/splice/delete\`).
- **Its own technical wiki**: ~20k lines of reference docs (Mongo + PMS schemas,
  API, frontend). The ~9.9k-line PMS schema reference was generated by a
  14-agent workflow and indexed into 441 RAG chunks. Reachable via semantic/BM25
  research AND direct disk \`wiki_grep\`/\`wiki_read\`.
- **Read-only + tenant-isolation invariants** the LLM can't override; the brain
  can reach ANY client collection/table via runtime discovery, not an allowlist.
- **Cost engineering**: Anthropic prompt-cache placement → ~73% warm-turn cost
  cut (≈$0.139 → ≈$0.037), 83–95% cache reads, cache-aware per-turn accounting.
- **In-house \`kibana_trace\`** observability package (~1,368 LOC) replacing Opik —
  one Elasticsearch span doc per traced call.
- **SSE hardening**: token-by-token reasoning, extended-thinking budget, live
  plan checklist, mid-stream client-disconnect cancellation, NGINX tuning.
- **Eval**: a 121-item adversarial capability exam + agentic-vs-legacy harness
  with an LLM judge; the React \`AgentActivity.tsx\` (727 LOC) renders the brain's
  live activity.

**Agentic Architecture (current):**
\`\`\`
User → Brain (plans) → delegate() → parallel worker clones [mongo|sql|faq]
                     → research() / wiki / workspace / compute → finalize() → SSE
\`\`\`
`,
};
...
- Downloads in background, swaps in-place after the user closes
- Dual release channels (stable + opt-in pre-release) with code-signing

**PRODUCTION INFRA KESHAV OWNS (the deeper engineering):**

Note on attribution: vf-simulation is a multi-author repo. The original
persona/replay/widget core was authored by a teammate (Neel); the multi-
modality (SMS/WhatsApp/call) engines by others (Rajat/Sarthk). Keshav
hardened that core and built nearly all the surrounding production
infrastructure — he owns ~74% of the codebase by current line count (143
commits). Specifically his:

- **Single-pool Chrome architecture** (\`bot/browserPool.ts\`): replaced per-
  replay \`puppeteer.launch()\` with one Chrome hosting N isolated
  BrowserContexts. 8 concurrent replays dropped from **58 processes / ~7.9 GB
  RAM to 15 processes / 2.5 GB**, eliminating a **32% bot-timeout failure rate**
  from CPU starvation. This was the key scaling fix.
- **Google OAuth + admin backend** (\`secrets/googleAuth.ts\`,
  \`secrets/superAuthDb.ts\` ~1,200 LOC, \`AdminPage.tsx\`): user activation,
  permissions, and session revocation — replacing the older admin-password /
  TOTP enrollment scheme.
- **S3 auto-update system from scratch** (\`updater/\`): stable + pre-release
  channels, signing, and a heartbeat-piggyback so running clients discover a
  release in ~30s instead of a 30-minute poll.
- **Elasticsearch tracing layer** (\`tracing/\`, 8 files, ~1,900 LOC) writing one
  ES doc per LLM/function span, plus **S3 audit logging** of every OpenAI call
  via a \`trackedCompletion\` wrapper.
- **The MCP "Investigate" server** (\`mcp/\`, ~4,000 LOC) — entirely his —
  exposing MongoDB transcripts + Elasticsearch traces to Claude Code / Cursor.
- **Agent Dispatch bridge** (\`agentDispatch/\`, ~2,000 LOC): a one-click action
  that opens a repo in Cursor and launches Claude Code with a pre-filled debug
  prompt, via a \`verbasuper://\` protocol handler.
- **Production-safety PII markers** (\`engine/prodTestMarker.ts\`): stamps
  simulated personas with the literal word \`test\` at the IPC boundary (so an
  operator override can't disable it), keeping synthetic leads filterable out of
  real customer dashboards.
- **Persona quality guards**: field-mismatch correction, an echo guard (stops
  the persona parroting the bot), and anti-loop hardening.
- **VerbaSuper 2.0** complete UI redesign + design system, theming, toasts, the
  in-app changelog, and an evals dashboard ported into the Activity tab.

**What Keshav Built (summary):**
- Single-pool Chrome scaling fix (~3× resource cut)
- MCP "Investigate" server (~4,000 LOC) for AI-assisted RCA
- S3 auto-update with dual channels + Google-OAuth admin backend
- Elasticsearch tracing + S3 audit logging
- Agent Dispatch (Cursor + Claude Code bridge)
- PII safety markers + persona-quality guards
- VerbaSuper 2.0 UI redesign, theming, ClickUp QA workflow
`,
};
...
- Select prompt template, inject variables (Jinja2, rendered server-side)
- Compare up to 4 models side-by-side across 14 providers
- See response with timing; "Open in Playground" prefills a production call

**MAY–JUNE 2026: THE PLATFORM ERA**

This is the densest stretch (114 commits in May alone) — the tool grew from an
eval toolkit into a full LLM engineering platform:
- **Service Replay** (\`logic/trace_replay.py\`, ~4,567 LOC — the largest module):
  re-fire historical production traces (root or sub-span) against a live
  deployed service across DEV→PROD, up to **20,000 traces in one sweep**, with
  SSE keepalives, dry-run, \`runs/trace\` (1–20) for nondeterminism detection, and
  Mongo-persisted run history.
- **Tool-calling agent loop** (\`logic/playground_tool_loop.py\`): runs the real
  model→tool→model loop against live \`vfai-bakery\` deploys (8-iteration cap), in
  both single and bulk runs.
- **Git Sync** (\`logic/git_prompts.py\`, ~1,457 LOC): commit / branch / open-PR
  prompt edits straight to the \`vf-prompts\` repo (validated Jinja + config.json,
  S3-audited, GitHub OAuth \`repo\` scope).
- **Vault**: save traces as frozen span snapshots that outlive ES retention,
  with threaded per-trace/per-span comments and org-internal share links.
- **AI Assistant** (\`logic/ai_explain.py\`, ~2,646 LOC): a tool-using model loop
  (get_page_state, get_entity, search_traces, navigate, …) over the platform.
- **Call Debugging**: read-only Vapi + Mongo + ES surface for a single voice
  call (hydrated transcript, audio, rendered prompt, per-tool deep-links).
- **Platform polish**: ⌘K command palette + vim-style \`g\`-leader nav, 30 per-page
  manuals the AI Assistant reads, a Celery/Redis background-jobs subsystem (with
  in-process fallback), a 15-palette theme system, "Open in Google Sheets" on
  exports, and a memory-optimization pass that cut idle-pod baseline ~150 MB.

**Scale:** a 236-endpoint FastAPI backend on a pure-Python \`logic/\` core +
Next.js 16 / React 19 frontend (~190 components) — ~170K LOC total, ~85% his.

**What Keshav Built:**
- The benchmark pipeline (extract traces → re-run prompts → LLM judge)
- A 236-endpoint FastAPI backend + Next.js 16 / React 19 frontend
- Trace Explorer, span-error explorer, conversations + tool-calls views
- LLM Playground: 4-way model compare across 14 providers, server-side Jinja2
- Bulk validation (5–500 traces) and Service Replay (up to 20,000 traces)
- Tool-calling agent loop against live deploys
- Git-Sync prompt → PR, the saved-trace Vault, the tool-using AI Assistant
- GitHub OAuth + Mongo sessions, Celery/Redis jobs, AWS Secrets Manager
`,
};
...
const projectVerbafloPlayground = {
  ...
  title: "VerbaFlo Playground — LLM Engineering Platform",
  content: `
Topics covered here: verbaflo playground, llm engineering platform, prompt
evaluation, llm evaluation, benchmark, a/b testing, service replay, trace
explorer, git sync prompts, model comparison, opik traces, eval toolkit.

**verbaflo-playground** began as VerbaFlo's internal tool for benchmarking
prompts against production conversation traces and grew into a full LLM
engineering platform (trace explorer, multi-provider playground, service
replay, prompt eval, Git-Sync prompts, and a tool-using AI assistant).
...
Primary author (~85% of ~285 commits): the benchmark pipeline, the 236-endpoint
FastAPI backend + pure-Python logic core, the Next.js 16 / React 19 frontend
(~190 components), the Trace Explorer, the multi-provider Playground (4-way
compare across 14 providers), Bulk validation, Service Replay (up to 20,000
traces), the tool-calling agent loop, Git-Sync prompts→PR, the Vault, and the
AI Assistant. The key insight: use production data, not synthetic test cases —
real conversations have edge cases you'd never think to test. (~170K LOC total;
a few teammates contributed minor observability charts and the initial scaffold.)
`,
};

const projectVerbafloPlaygroundDeepDive = {
  id: "project-verbaflo-playground-deep-dive",
  title: "VerbaFlo Playground — LLM Engineering Platform (Full Deep Dive)",
  content: `
Topics covered here: verbaflo playground deep dive, llm engineering platform,
trace explorer, llm playground, model comparison, 14 providers, service replay,
trace replay, bulk runs, llm judge, prompt evaluation, git sync prompts, prompt
to PR, vault, ai assistant, tool-using agent, call debugging, vapi, verbasuper
telemetry, fastapi, nextjs, observability, litellm, what keshav built at
verbaflo, biggest project, full ownership platform.

**verbaflo-playground** is VerbaFlo's internal **LLM engineering platform** —
the single tool the AI team uses to observe, debug, replay, evaluate, and
improve the prompts behind their production conversational agents (chat + voice),
using real production traffic as the test bed. Keshav is the **primary author
(~85% of ~285 commits, Mar–Jun 2026)**; it started as a narrow "benchmark a
prompt change against production data" utility and grew into a full platform.

**SCALE (one of his largest projects):**
- ~**170K lines of code** total: ~47K Python (FastAPI + a pure-Python \`logic/\`
  core) and ~123K TypeScript/TSX (a Next.js 16 / React 19 frontend, ~190
  components).
- A **236-endpoint FastAPI backend** (\`api.py\`, ~11.8K LOC) on \`:25503\`, sharing
  the \`logic/\` core with the Next.js frontend on \`:3000\` (proxied via
  \`frontend/proxy.ts\`). The original Streamlit UI it was born on is deprecated.
- ~25 docs in \`docs/\` plus **30 per-page user manuals** that the in-app AI
  Assistant reads at runtime.

**THE AI ASSISTANT (the tool-using RAG/agent feature):**
\`logic/ai_explain.py\` (~2,646 LOC) is a context-aware, tool-using model loop —
ask it anything about what you're looking at and it investigates the live system
to answer. Its tools include \`get_page_state\` (what the user is currently viewing),
\`get_entity\` (resolve a lead/trace/prompt), \`search_traces\`, \`get_vault_comments\`,
and \`navigate\` (it can move the UI for you). It reads the 30 per-page manuals as
grounding context, so it understands every screen. Default model
\`gemini-3.1-flash-lite\`. This is the platform's built-in "explain + investigate"
agent — distinct from the agent's data being fed into VerbaFlo's other RAG
pipelines.

**SERVICE REPLAY (\`logic/trace_replay.py\`, ~4,567 LOC — the largest module):**
Pull historical production traces (root traces, or tool/processor sub-spans) and
**re-fire each against a live deployed VF-AI service** (DEV / TEST / STAGING /
PROD / US-PROD). Supports **mass-validation sweeps of 1–20,000 traces** with SSE
keepalives for long runs, \`runs/trace\` (1–20 re-runs of the same input) for
hallucination / nondeterminism detection, a dry-run mode, and Mongo-persisted run
history. This is how the team proves a deploy didn't regress before it ships.

**LLM PLAYGROUND (\`logic/llm_playground.py\`):**
Monaco editors for system + user prompts with **live server-side Jinja2 rendering**,
response_format / tools / sampler-knob config, SSE streaming, and **side-by-side
comparison of up to 4 models in one run**. Routes to **14 providers** via LiteLLM:
OpenAI, Anthropic, Gemini, Groq, Cerebras, Azure, Mistral, Together AI, DeepSeek,
xAI, Cohere, Fireworks AI, OpenRouter, Perplexity. "Open in Playground" from any
trace prefills system+user templates, rendered variables, response_format, tools,
model, sampler knobs, and prompt_key/client_code — reproducing a captured
production call with edits.

**BULK / BATCH RUNS (\`logic/playground_bulk.py\`, ~3,047 LOC):**
Run a new prompt against **5–500 historical traces** matched by \`prompt_key\`, with
per-row diffs, AI semantic-match scoring, criteria scoring, "edit & rerun on the
same data" replays, and Excel / Google-Sheets export.

**TOOL-CALLING AGENT LOOP (\`logic/playground_tool_loop.py\`):**
When a prompt declares tools, it runs the real model→tool→model loop against a live
\`vfai-bakery\` deploy (\`POST /v1/tool/{name}\`), feeding results back until a final
answer or an 8-iteration safety cap — mirroring exactly what production does, in
both single and bulk runs.

**TRACE EXPLORER + OBSERVABILITY (\`logic/trace_es_client.py\`, ~3,575 LOC):**
Browse traces across Elasticsearch environments with filters and content search,
waterfall + flow-graph views, conversations-by-lead grouping, a tool-calls view,
a span-errors view, and trace-statistics aggregations.

**LLM-AS-A-JUDGE EVALUATION (\`logic/evaluation_judge.py\`, \`eval_runner.py\`,
\`unified_eval_generator.py\`):** similarity and correctness scoring, plus
auto-generated pass/fail eval categories per prompt key.

**GIT SYNC (\`logic/git_prompts.py\`, ~1,457 LOC):**
Diff edited prompts against the \`vf-prompts\` repo and **commit / branch / open a
PR directly from the browser** (GitHub OAuth \`repo\` scope), with Jinja2 +
config.json pre-commit validation and S3 audit events. Edit a prompt, see it pass
validation, and ship it as a PR without leaving the tool.

**VAULT (\`logic/vault.py\`):** save traces as frozen span snapshots that outlive ES
retention, with threaded per-trace and per-span comments and org-internal share
links.

**CALL DEBUGGING (\`logic/calls/\`):** a read-only Vapi + Mongo + ES surface for a
single voice call — hydrated transcript with bot-turn provenance, audio, the
rendered system prompt, and per-tool LLM deep-links.

**VERBASUPER TELEMETRY (\`logic/verbasuper/\`):** read-only dashboards over the evals
MongoDB (KPIs, dimensions, failure clusters, usage events).

**PLATFORM POLISH:** a ⌘K command palette + vim-style \`g\`-leader chord nav,
GitHub-OAuth auth with Mongo sliding sessions and roles/bans, a Celery/Redis
background-jobs subsystem (with in-process fallback) for exports/bulk/replay, a
15-palette theme system with an AI palette generator, "Open in Google Sheets" on
every export, and a memory-optimization pass that deferred heavy imports to cut
the idle-pod baseline by ~150 MB.

**DEVELOPMENT ARC:**
1. **Foundation (March, Streamlit):** a Snowflake/Opik extractor → Jinja
   prompt-runner → LLM judge, with the pivotal split of \`logic/\` (pure source of
   truth) from the UI, and FastAPI exposing the logic.
2. **Observability + Next.js migration (late Mar–Apr):** the Trace Explorer, batch
   export, multi-environment (UK/US) support, and the React frontend that ports
   the playground, structured-output schemas, span-error explorer, and bulk runs.
3. **Hardening (Apr–May):** GitHub OAuth + Mongo sessions, the memory pass, a
   self-stats pod-metrics page, role/ban management, a 25-model catalog expansion.
4. **The platform era (May, 114 commits):** Git Sync, Call Debugging, Conversations
   view, VerbaSuper telemetry, Service Replay, the ⌘K palette + entity resolver,
   the tool-using AI Assistant, page manuals, theming, and nondeterminism detection.
5. **Agentic + ops maturity (late May–Jun):** background jobs, the playground
   tool-calling agent loop, Google Sheets export, the Vault, and a bulk-run agent
   loop — enforced by a real-hit \`tests/api_deep/\` integration suite.

**TECH STACK:** Python / FastAPI / Uvicorn / Pydantic v2, LiteLLM (14 providers) +
Instructor, Jinja2, Celery + Redis, boto3 (AWS Secrets Manager + S3); Elasticsearch,
MongoDB, Opik, Vapi; Next.js 16 / React 19, TanStack Query + Virtual, Zustand,
Monaco, Recharts, Mermaid, shadcn/Tailwind v4, dnd-kit. Shares trace data with the
simulation MCP and the copilot's kibana_trace.

**WHY IT MATTERS:** before this, "is this prompt change better?" was guesswork.
Now the team can compare models, replay thousands of real conversations against a
candidate deploy, judge the diffs automatically, and ship the winning prompt as a
PR — all from one tool, on production data.

Repo: verbaflo-playground (internal).
`,
};
...
export const CORPUS_SECTIONS = [
  ...
  projectVerbafloPlaygroundDeepDive,
